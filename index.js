...
const vcard = 'BEGIN:VCARD\n' // Tarjeta de contacto
            ...
            + 'FN:LuBot\n' // Nombre
            + 'ORG:Shanduy;\n' // Propietario
            + 'TEL;type=CELL;type=VOICE;waid=525586805544:\n' // ID de WhatsApp + número de teléfono
            + 'END:VCARD'
/******END OF VCARD INPUT******/
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Hola @${num.split('@')[0]}\n bienvenido, Presentate *${mdata.subject}*`
				...
				teks = `Salio por joto @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
			...
			mess = {
				...
				only: {
					...
					Badmin: '[❗] Este comando solo se puede usar cuando el bot se convierte en administrador! ❌',
                                        daftarB: `── 「REGISTRATE CRACK��\n\nHOLA !\n\nNo estas registrado en mi base de datos 😳 \n\nComando : ${prefix}daftar Nombre\nEjemplo : ${prefix}daftar Felix
`,
				}
			}
			...
			const ownerNumber = ["525586805544@s.whatsapp.net"] // replace this with your number
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '525586805544@s.whatsapp.net'
                        /******ApiKey Input******/
